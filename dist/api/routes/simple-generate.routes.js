 ...
 */
router.post('/', (0, validate_request_1.validateRequest)({ body: validate_request_1.commonSchemas.generateCode }), async (req, res) => {
    try {
        ...
        logger_1.logger.info('Generating code from prompt with user-provided API key', { provider, model, conversationId });
...
Generate complete, runnable code. The user expects a full project they can immediately run.`;
        ...
        const verifiedCode = (0, code_verifier_1.verifyAndCleanCode)(generatedCode);
        ...
        logger_1.logger.info('Code generated and verified', {
            provider,
            model,
            tokensUsed,
        ...
        if (!verifiedCode.isValid) {
            logger_1.logger.warn('Code verification found issues', { errors: verifiedCode.errors });
        }
        return res.json({
            status: 'success',
            data: {

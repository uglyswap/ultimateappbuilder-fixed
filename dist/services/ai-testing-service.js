...
exports.aiTestingService = exports.AITestingService = void 0;
...
class AITestingService {
     ...
     */
    async generateUnitTests(code, fileName, framework = 'jest') {
        logger_1.logger.info(`Generating unit tests for ${fileName} using ${framework}`);
...
Generate ONLY the test code, nothing else.`;
     ...
     */
    async generateIntegrationTests(apiEndpoints, framework = 'jest') {
        logger_1.logger.info('Generating integration tests');
...
Generate comprehensive integration test suite.`;
    ...
    async generateE2ETests(userFlows, framework = 'playwright') {
        logger_1.logger.info(`Generating E2E tests using ${framework}`);
...
Generate complete E2E test suite.`;
     ...
     */
    async generateComponentTests(componentCode, componentName) {
        logger_1.logger.info(`Generating component tests for ${componentName}`);
...
Generate comprehensive component tests.`;
     ...
     * Generate API tests
     */
    async generateAPITests(apiSpec) {
        logger_1.logger.info('Generating API tests from spec');
...
Generate complete API test suite.`;
     ...
     */
    async generatePerformanceTests(endpoints) {
...
${endpoints
            .map(endpoint => `  it('should handle load on ${endpoint}', async () => {
...
`)
     ...
     */
    async generateAccessibilityTests(pages) {
        logger_1.logger.info('Generating accessibility tests');
        const testCode = `import { test, expect } from '@playwright/test';
import { injectAxe, checkA11y } from 'axe-playwright';

${pages
            .map(page => `test('${page} should be accessible', async ({ page }) => {
...
afterEach(() => {
  cleanup();
    ...
    async runTests(config) {
        logger_1.logger.info(`Running ${config.type} tests with ${config.framework}`);
...
exports.aiTestingService = new AITestingService();

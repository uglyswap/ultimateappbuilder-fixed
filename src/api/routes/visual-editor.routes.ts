...
import { visualEditorService, COMPONENT_LIBRARY } from '@/services/visual-editor-service';
...
router.get('/components', async (req: Request, res: Response) => {
  try {
    ...
    if (category && typeof category === 'string') {
      const components = visualEditorService.getComponentsByCategory(category);
      return res.json({
        success: true,
        components,
      });
 ...
 */
router.post('/design/:designId/component', async (req: Request, res: Response) => {
  try {
    ...
    const component = await visualEditorService.addComponent(
      designId,
      componentType,
      parentId,
      props
 ...
 */
router.post('/design/:designId/generate-code', async (req: Request, res: Response) => {
  try {
    ...
    const { componentCode, routeConfig } = await visualEditorService.generateCodeFromDesign(
      design
    );
 ...
 */
router.post('/design/:designId/save', async (req: Request, res: Response) => {
  try {
    const { design } = req.body;
...
router.post('/generate-from-description', async (req: Request, res: Response) => {
  try {
    ...
    const design = await visualEditorService.generatePageFromDescription(
      projectId,
      pageName,
      description
    );

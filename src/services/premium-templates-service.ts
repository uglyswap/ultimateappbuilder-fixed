...
export class PremiumTemplatesService {
  ...
  async seedPremiumTemplates() {
    logger.info('Seeding premium templates...');

    const categories = await this.createCategories();
    const templates = await this.createPremiumTemplates(categories);

    logger.info(`Seeded ${templates.length} premium templates across ${categories.length} categories`);
  ...
  private async createPremiumTemplates(categories: any[]) {
    const templates = [
      // SaaS Templates
      {
...
export const premiumTemplatesService = new PremiumTemplatesService();
